...
#define DP_PIN  17 // DP of 16segment LED
...
boolean shoutKeystrokes = false;    // should be false in japan, because the module has not GITEKI.
...
char gotKeystroke(uint8_t* p)
{
  ...
  // displays to 16segment LED
  display_char_to_16segment_led(letter);
...
void scan()
{
  ...
  while (1)
  {
    if (channel > 80){
      channel = 3;
      /* // serch fifth bytes of device id.
      p4_code++;
      if (p4_code > P4_CODE_STOP){
        p4_code = P4_CODE_START;
      }
      */
    }
    display_char_to_16segment_led(channel % 10 + 0x30);     //display ch to 16seg led. (shift 0x30 for ASCII Code)
    set_dp(HIGH);                                           //turn on DP.
    ...
    {      
      if (radio.available())
      {
        ...
        // 0xCD -> samy's (English), 0xCE -> Horiai's (Japanese), 0xCF -> my keyboard has.
        if ((p[4] == 0xCD) || (p[4] == 0xCE) ||(p[4] == 0xCF))
        {
          ...
          { 
            ...
            display_char_to_16segment_led(0x20);     //16seg led to be blank, turn off DP.
...
void setup()
{
  ...
  spl("KeySweeperCF");
  init_i2c_16seg(0, DP_PIN);
  display_str_to_16segment_led("K e y S w e e p e r C F  ", 200);
